...
namespace Cosolem
{
    public partial class frmIndicesFinancieros : Form
    {
        ...
        private void frmIndicesFinancieros_Load(object sender, EventArgs e)
        {
        ...
        private void CargarProductos()
        {
            ...
            ((Modelo)cmbModelo.SelectedItem).tbProducto.Where(x => x.estadoRegistro).ToList().ForEach(x =>
            {
                ...
                x.idIndiceFinanciero = 0;
                x.indiceFinanciero = 0;
                x.idCosto = 0;
                x.costo = 0;
                ...
                if (_tbIndiceFinanciero != null)
                {
                    ...
                    x.fechaHasta = _tbIndiceFinanciero.fechaHasta;
                }
        ...
        private void tsbGrabar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_BindingListtbProducto.Where(x => x.fechaHasta.Date < Program.fechaHora.Date).Any()) mensaje += "Indice financiero con fecha de vigencia menor a la fecha actual\n";

                if (String.IsNullOrEmpty(mensaje.Trim()))
                {
                    _BindingListtbProducto.ToList().ForEach(x =>
                    {
                        tbIndiceFinanciero _tbIndiceFinanciero = (from IF in _dbCosolemEntities.tbIndiceFinanciero where IF.idIndiceFinanciero == x.idIndiceFinanciero select IF).FirstOrDefault();
                        if (_tbIndiceFinanciero == null)
                        {
                            _tbIndiceFinanciero = new tbIndiceFinanciero();
                            _tbIndiceFinanciero.idEmpresa = idEmpresa;
                            _tbIndiceFinanciero.idProducto = x.idProducto;
                            _tbIndiceFinanciero.indiceFinanciero = x.indiceFinanciero;
                            _tbIndiceFinanciero.fechaDesde = Program.fechaHora.Date;
                            _tbIndiceFinanciero.fechaHasta = x.fechaHasta.Date;
                            _tbIndiceFinanciero.estadoRegistro = true;
                            _tbIndiceFinanciero.fechaHoraIngreso = Program.fechaHora;
                            _tbIndiceFinanciero.idUsuarioIngreso = idUsuario;
                            _tbIndiceFinanciero.terminalIngreso = Program.terminal;
                            _dbCosolemEntities.tbIndiceFinanciero.AddObject(_tbIndiceFinanciero);
                        }
                        else
                        {
                            _tbIndiceFinanciero.indiceFinanciero = x.indiceFinanciero;
                            _tbIndiceFinanciero.fechaHoraUltimaModificacion = Program.fechaHora;
                            _tbIndiceFinanciero.idUsuarioUltimaModificacion = idUsuario;
                            _tbIndiceFinanciero.terminalUltimaModificacion = Program.terminal;
                        }
                    ...
                    frmIndicesFinancieros_Load(null, null);
                }
                else
